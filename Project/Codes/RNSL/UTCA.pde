      ...
      for(int j=0;j<rows;j++) {
        if(board[i][j].previous==1) {
          ...
          if(x==-1 || x>v+1) {
            ...
            if(board[i][(j+1)%rows].previous==0) {
              int y = -1;
              for(int k=1;k<columns;k++) {
                if(board[(i+k)%columns][(j+1)%rows].previous!=0) {
                  y = k;
                  break;
                }
              }
              if(y==-1 || y>x) {
                int yBack = -1;
                for(int k=1;k<columns;k++) {
                  if(board[(i-k+columns)%columns][(j+1)%2].previous!=0) {
                    yBack = k;
                    break;
                  }
                }
      ...
      for(int y=0;y<rows;y++) {  
        ...
        if(board[x][y].previous==1) {
          if(board[x][y].change) {
            ...
            //System.out.println("change: " + x + " " + y + " " + next);
            //System.out.println("Change from " + y + " to " + (y+1)%rows);
          }
          else {
            board[x][y].setState(0);
            int v = board[x][y].v;
